...
# Usage: bash .claude/skills/importing-amq-songs/scripts/check_shows.sh <amq_export.json>

if [ -z "$1" ]; then
  echo "Usage: bash .claude/skills/importing-amq-songs/scripts/check_shows.sh <amq_export.json>"
  exit 1
fi
