...
- Each song with: name, artist, level (display = stored + 1), wait days, shows (sourced from play_history) with grouped clickable media URLs per show
...
- Show data and media URLs in the HTML report are sourced directly from `play_history` (not `rel_show_song`), grouped by show with duplicate URLs deduplicated
